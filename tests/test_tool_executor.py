...
# ---- 副作用工具跨 turn 防连点（策略 B） ----

def _side_effect_executor(window: float = 60.0):
    rag_tool = MagicMock(); rag_tool.run.return_value = []
    order_service = MagicMock()
    order_service.modify_address.return_value = {"ok": True, "order_id": "A1001", "new_address": "北京"}
    return ToolExecutor(
        order_service=order_service,
        logistics_service=_fake_logistics_service(),
        handoff_service=_fake_handoff_service(),
        refund_service=RefundService(),
        rag_tool=rag_tool,
        side_effect_dedup_window=window,
    ), order_service


def test_side_effect_cross_turn_blocked(tmp_path=None):
    """跨 turn 防连点：新 turn（tool_cache 已重置）内重复提交相同副作用调用，直接拒执行。

    模拟用户连点「改地址」——第一轮已办理，第二轮（新图运行）再点同一条，应被去重、
    不重复触发，且明确告知用户已提交。
    """
    executor, order_service = _side_effect_executor(window=60.0)
    call = {
        "id": "c", "type": "function",
        "function": {"name": "modify_address", "arguments": '{"order_id": "A1001", "new_address": "北京"}'},
    }
    state = _state()
    # turn 1
    asyncio.run(executor.run([call], state))
    assert order_service.modify_address.call_count == 1
    assert len(state.side_effect_log) == 1
    # 模拟新 turn：input_normalizer 会重置 tool_cache（但不重置 side_effect_log）
    state.tool_cache = {}
    # turn 2：同一条副作用调用，应在窗口内被拒执行
    asyncio.run(executor.run([call], state))
    # handler 只真正执行一次（跨 turn 未重复触发）
    assert order_service.modify_address.call_count == 1
    # 拒执行结果作为 error 入 tool_results，交由生成节点告知用户
    assert state.tool_results[-1].kind == "error"
    assert "去重" in state.tool_results[-1].raw_result["message"]


def test_side_effect_cross_turn_allowed_after_window():
    """窗口过期后（window=0，记录立即失效）允许再次执行，不误伤主动重办。"""
    executor, order_service = _side_effect_executor(window=0.0)
    call = {
        "id": "c", "type": "function",
        "function": {"name": "modify_address", "arguments": '{"order_id": "A1001", "new_address": "北京"}'},
    }
    ...
    asyncio.run(executor.run([call], state))
    # 窗口已过 → 重新执行
    assert order_service.modify_address.call_count == 2


def test_side_effect_cross_turn_different_args_allowed():
    """不同参数视为不同操作（改到不同地址），跨 turn 仍各自执行，不误杀。"""
    executor, order_service = _side_effect_executor(window=60.0)
    call1 = {
        "id": "c1", "type": "function",
        "function": {"name": "modify_address", "arguments": '{"order_id": "A1001", "new_address": "北京"}'},
    ...
    call2 = {
        "id": "c2", "type": "function",
        "function": {"name": "modify_address", "arguments": '{"order_id": "A1001", "new_address": "上海"}'},
    }
    state = _state()
    asyncio.run(executor.run([call1], state))
    state.tool_cache = {}
    asyncio.run(executor.run([call2], state))
    assert order_service.modify_address.call_count == 2


def test_non_side_effect_not_logged_cross_turn():
    """只读工具不记 side_effect_log，跨 turn 重复仍正常执行（允许主动重查）。"""
    rag_tool = MagicMock(); rag_tool.run.return_value = []
    logistics_service = _fake_logistics_service()
    executor = ToolExecutor(
        order_service=_fake_order_service(),
    ...
    state = _state()
    asyncio.run(executor.run([call], state))
    state.tool_cache = {}  # 模拟新 turn
    asyncio.run(executor.run([call], state))
    # 只读工具跨 turn 照常执行两次，且不写入 side_effect_log
    assert logistics_service.get_logistics.call_count == 2
    assert state.side_effect_log == []
