...
**权衡**：Qdrant `Modifier.IDF` 仅做 `IDF·TF` 点积，不做 Okapi `k1`/`b` 饱和与长度归一，故失去运行时 `bm25_k1`/`bm25_b` 调参（`RagConfig` 已移除）。其中 **k1** 控制词频饱和（抑制同一词反复出现刷分），**b** 控制长度归一（抵消长文档因 token 多而占优）；以「服务端打分 + 跨 worker 一致 + 更少代码」换此能力，对短且长度相近的 RAG chunk 影响很小。
