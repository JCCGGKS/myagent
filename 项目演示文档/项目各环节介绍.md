...
| 检索（向量语义 + 关键词 BM25） | 内存自研 BM25（倒排索引）+ Qdrant 向量语义 | 内存 BM25（正则分词，中文按字切，IDF/avgdl 现算）+ Qdrant 语义，两路并行召回做 RRF 融合 |
...
> RAG 分**离线（入库）**与**在线（检索）**两阶段：Qdrant 存 `dense`（语义召回），BM25 走**进程内存倒排索引**。**仅 `hybrid` 策略**下 BM25 + 语义两路并行召回并经 **RRF 融合**；`bm25` / `semantic` 单策略各自独立召回、不做融合。详见下方「BM25 原理与实现」与「① 离线 / ② 在线」两节。

### BM25 原理与实现

**原理（内存倒排索引，IDF/avgdl 现算）**：打分 = `Σ IDF(t)·[TF(t,d)·(k1+1) / (TF(t,d)+k1·(1−b+b·|d|/avgdl))]`——**TF(t,d)** 词在本文档出现次数（文档本地量），**IDF(t)=log((N−n+0.5)/(n+0.5)+1)**（N=索引文档数、n=含该词的文档数，索引全局量）。手搓 `BM25Index` 在查询时**现算 IDF/avgdl**（`k1`/`b` 由 `RagConfig.bm25_k1`/`bm25_b` 注入），分数即标准 BM25（含 TF 饱和 + 长度归一）；入库时把 chunk 的 TF 计进内存倒排索引，无需把全局 IDF 烤进每个 point。

**当前实现（手搓原生 BM25）**：入库时**同步把 chunk 写进进程内存倒排索引**（`rag/retrieval/bm25.py`），在线检索主路径走该内存索引做**真·BM25**，不再受 Qdrant 伪 BM25 近似限制。Qdrant 的 `bm25` 稀疏向量**仅在纯 BM25 模式（无 embedding）下作为 point 载体保留**，不用于检索；有 embedding 的 semantic/hybrid 分支只写 `dense`。进程重启 / 其它 worker 首次查询时，内存索引为空，经 `rebuild_from_qdrant` 一次性从 Qdrant 的 `content` payload 重建。
...
flowchart TD
    ...
    PRE{"retrieval_strategy?\nsemantic/hybrid 但无 embedding"}
    ...
    QD[("Qdrant\n{dense: VectorParams(Cosine)}\n[bm25: SparseVectorParams · 仅纯 BM25 模式载体]")]

    UP --> ID --> PRE
    PRE -- "缺失 → ERROR+400" --> ENDX([拒绝入库])
    PRE -- "通过" --> ING --> CH --> CK
    CK --> EMB
    CK --> SP
    CK --> IDX["BM25Store.add()\n同步写内存倒排索引\n(get_bm25_store)"]
    EMB --> UPQ
    SP --> UPQ
...
| --- | --- | --- | --- |
| markdown | `MarkdownChunkingStrategy` | 标题块 + 问答对 + 递归字符兜底；`chunk_type=text` | 结构为主、问答优先（`Q:`/`答：` 每对一块）、字符兜底；继承 `heading_path` |
| word | `WordChunkingStrategy` | 同 Markdown（parser 先抽 `.docx` 成标题树纯文本，复用 `structure_chunk`） | 标题层级 / 问答 / 字符，与 Markdown 同源 |
| json | `JsonChunkingStrategy` | 每记录按 `text_field` 切块（`content` 递归字符切）；`chunk_type=text` | 每块 `metadata.source` = 记录字段 `json.dumps`；FAQ `{question,answer}` 也按记录切 |
| excel / csv | `ExcelCsvChunkingStrategy` | 行级（每数据行一块，重建 `列名=值`）+ 表概要块；`chunk_type=table` | 带 `table_id` / `caption` / `header` / `row_index`；表概要块负责「找表」 |
| pdf | `PdfChunkingStrategy` | 含表格→行级（同 Excel，`table`）；长文档 / 合同→条款级（`clause`） | 表格块委托 Excel 策略；条款按 `第X条` / `（一）` / `1.` 切；跨页由 parser 缝合 |
| ppt | `PptChunkingStrategy` | 按版式分栏 / 段落（`---` 或空行）；页内条款 / 要点标记→`clause`，否则 `text` | 跨页版式块由 parser 按阅读序缝合 |
...
```mermaid
flowchart TD
    MEM[("BM25Store\n内存倒排索引 BM25Index\nset 空集时 rebuild_from_qdrant 重建")]
    QD[("Qdrant\n{dense: VectorParams(Cosine)}\n[bm25: SparseVectorParams · 仅纯 BM25 模式载体]")]
...
> **离线要点**：分块用策略模式（markdown 结构为主、问答优先、字符兜底；excel/csv 一行一 chunk 带 `table_id`）；入库前校验 embedding 配置（semantic/hybrid 缺失直接 400）；无 embedding 时退化写 BM25-only（纯 BM25 模式，Qdrant 以 `bm25` 稀疏向量作 point 载体供重建，不参与检索）。上传/更新/删除直连 `get_qdrant_client()`，幂等由 `content_hash` 去重。
>
> **在线要点**：Hybrid 用 **RRF 融合**（`rrf_k` 默认 60，与量纲无关，可融合 BM25 与 cosine 0~1）；BM25 主路径走**内存倒排索引**做真·BM25（`IDF`/`avgdl` 查询时现算，分数即标准 BM25），不再依赖 Qdrant 稀疏检索、不限定 0~10；`min_score_threshold` 是单一共享字段但三策略量纲不同，前端需按 `retrieval_strategy` 限幅（默认 0.0=不过滤）；rerank 失败静默降级原序，但 semantic/hybrid 缺 embedding 会快速失败（RuntimeError）。
>
...
评估本身即「规则-only vs 规则+LLM」的对比迭代：

- 规则-only 准确率 72.60%（726/1000）；叠加 LLM 兜底后 96.50%（965/1000），绝对提升 +23.90pp、多命中 239 条，LLM 兜底命中 261 次。
- 按类别增量：colloquial 0.00%→90.45%（+90.45pp）、finer_subintent 75.57%→93.67%（+18.10pp）；rule_hit / multiturn_followup / unrecognize 维持 100%。
- 残余：logistics.delayed ↔ logistics.not_received 子意图混淆；LLM 覆盖会反噬少数已正确的 rule_hit 与 unrecognize 样本（已知权衡）。
