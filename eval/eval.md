...
> **基线（1000 条，工具 schema 修复前，规则评估无 LLM 裁判）**：
...
>   ReAct 工具选择这一环。
>
> **修复后（1000 条，强化 `TOOL_SCHEMAS` 工具描述）**：
> - 节点路径 98.40%、意图 87.70%、动作/分支 **95.80%**、槽位抽取 100%、缺失槽位 100%、澄清 99.40%
> - **轨迹总准确率（六项全过）86.40%**
> - 响应时间：最短 0.011s / 最长 10.650s / 平均 4.673s / P50 4.700s
> - 动作准确率 **+29.3pp（66.50% → 95.80%）**，总准确率 **+25.3pp（61.10% → 86.40%）**；
>   订单↔物流、退款→转人工/知识的工具误选基本消除。剩余约 4% 动作误差 + 约 12% 意图
>   误识别构成总准确率的主要残余缺口（意图层未改，见 `template/评估调优/agent/04_轨迹评估优化.md`）。
> - 注意：基线→一轮非纯净 A/B（修复同时包含多处改动），数值为各自生效配置下的结果。
>
> **二轮优化后（1000 条；意图规则消歧 + 评估按类别分母修复）**：
> - 节点路径 99.40%、意图 **99.90%**、动作/分支 **98.20%**、槽位抽取 100%、缺失槽位 100%、澄清 99.40%
> - **轨迹总准确率（六项全过）98.20%**
> - 响应时间：最短 0.011s / 最长 13.951s / 平均 2.882s / P50 2.725s
> - 相对首轮修复再 **+11.8pp（86.40% → 98.20%）**；意图 **+12.2pp（87.70% → 99.90%）**，动作 **+2.4pp（95.80% → 98.20%）**。
> - 本轮主攻意图层：新增退款咨询优先规则（规则/政策/怎么退/流程等探索性表述归 `consult_policy`），
>   修复「七天无理由怎么退」「帮我退款规则」被误判 `request_refund`；补全投诉情绪关键词（气死/越想越气/态度）
>   修复投诉误路由。退款类 `refund_consult` / `refund_request` / `complaint` 均达 100%。
> - 残余缺口（约 1.8%）集中在多轮跟进（`multiturn_followup` 56%）与无单号澄清（`clarify_no_id` 80%），
>   且失败维度几乎全为「动作/工具」（路径与意图已正确）：`A1002 怎么样了` 类跟进金标本身不自洽
>   （同句不同期望工具）、`帮我催一下 / 物流信息` 误走 `handoff`。属金标缺陷 + P3 多轮边界，非核心意图问题。
>   另修复 `run_eval.py` 按类别表格误用全局分母的 bug（现为类别内分母）。详见 `template/评估调优/agent/04_轨迹评估优化.md`。
...
> **基线（1000 条，工具 schema 修复前）**：
> 关键事实 29.40%、意图 88.10%、动作/轨迹 56.40%、LLM 裁判通过率 37.40%（均分 0.472）；
> 响应时间 最短 0.735s / 最长 50.817s / 平均 6.806s / P50 5.644s。
> 主要问题：退款类过度转人工、订单/物流误选工具、轻微幻觉。
>
> **修复后（1000 条，强化 `TOOL_SCHEMAS` 工具描述）**：
> - 关键事实命中率 **69.30%**、意图 87.70%、动作/轨迹 **95.80%**
> - LLM 裁判通过率 **75.30%**（均分 **0.786**）
> - 工具选错（订单↔物流、退款→转人工/知识）已消除，动作层与轨迹层对齐（均 95.80%）。
> - 残余缺口已转向 **RAG 检索质量**：退款政策 `consult_policy` 类已正确选 `rag_retrieve`，
>   但检索返回弱/空，回复缺「七天无理由/原路退回」等事实点；另约 12% 意图仍误识别。
>
> **二轮优化后（1000 条；意图规则消歧后复跑）**：
> - 关键事实命中率 **59.80%**、意图 **99.90%**、动作/轨迹 **98.30%**
> - LLM 裁判通过率 **72.10%**（均分 **0.723**）
> - 响应时间：最短 0.032s / 最长 8.621s / 平均 2.816s / P50 2.663s
> - 意图/动作相对首轮修复 **再 +12.2pp / +2.5pp**（意图规则消歧生效）；
>   但 **key_facts / 裁判较首轮修复回落约 10pp / 3pp**：`agent_node` 在 `consult_policy` 下
>   倾向于误选 `order_query` 而非 `rag_retrieve`（如 `单号A1002，退款政策` 实际走 order_query），
>   导致退款政策事实点（七天/原路退回）缺失，`refund_consult` 关键事实 0%。
>   该回落是 **RAG 工具选择**问题，非意图层——意图/路径均正确。
> - 建议：用「意图→工具硬护栏」（P3）将 `consult_policy` 强制绑定 `rag_retrieve`，使答案质量稳定。
