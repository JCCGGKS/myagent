...
> **基线（1000 条，qwen3.7-max-preview，工具 schema 修复前，规则评估无 LLM 裁判）**：
...
>   ReAct 工具选择这一环。
>
> **修复后（1000 条，qwen3.7-max-2026-06-08，强化 `TOOL_SCHEMAS` 工具描述 + 切换支持关 thinking 的模型）**：
...
> - 注意：此对比非纯净 A/B——同时切换了模型（preview 强制 thinking 开且拒绝 `enable_thinking:False`；
>   2026-06-08 支持关 thinking）。两者均为当前生效的最终状态。
...
> **基线（1000 条，qwen3.7-max-preview，工具 schema 修复前）**：
...
>
> **修复后（1000 条，qwen3.7-max-2026-06-08，强化 `TOOL_SCHEMAS` 工具描述）**：
> - 关键事实命中率 **69.30%**、意图 87.70%、动作/轨迹 **95.80%**
> - LLM 裁判通过率 **75.30%**（均分 **0.786**）
> - 响应时间：最短 0.041s / 最长 11.009s / 平均 4.809s / P50 4.831s
> - 关键事实 **+39.9pp**、动作 **+39.4pp**、裁判通过率 **+37.9pp**、均分 **+0.314**；
>   工具选错（订单↔物流、退款→转人工/知识）已消除，动作层与轨迹层对齐（均 95.80%）。
> - 残余缺口（约 25% 裁判未过）已转向 **RAG 检索质量**：退款政策 `consult_policy` 类
>   已正确选 `rag_retrieve`，但检索返回弱/空，回复缺「七天无理由/原路退回」等事实点；
>   另约 12% 意图仍误识别（未改）。详见 `template/评估调优/agent/04_轨迹评估优化.md`。
