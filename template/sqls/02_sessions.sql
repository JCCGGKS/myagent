...
CREATE TABLE IF NOT EXISTS `sessions` (
  ...
  `current_intent`    VARCHAR(64)   DEFAULT NULL                                    COMMENT '当前主意图',
  `current_stage`     VARCHAR(64)   DEFAULT NULL                                    COMMENT '当前任务阶段',
  `risk_level`        VARCHAR(16)   DEFAULT NULL                                    COMMENT '风险等级（low / medium / high）',
  `summary`           TEXT          DEFAULT NULL                                    COMMENT '运行摘要（running_summary）',
  `handoff_required`  TINYINT(1)    NOT NULL DEFAULT 0                              COMMENT '是否需要转人工（0 否 / 1 是）',
