...
def _load_rag_config() -> RagConfig:
    """按 APP_ENV 基线 + local 覆盖 合并读取 rag 配置段。"""
    env_name = os.getenv("APP_ENV", "test").strip().lower() or "test"
    base_path = CONFIG_DIR / f"llm_config.{env_name}.yml"

    rag_data: dict[str, object] = {}
    for path in [base_path, LOCAL_OVERRIDE_PATH]:
        if path.exists():
            try:
...
def _persist(config: RagConfig) -> None:
    """把 rag 段写回 llm_config.local.yml（保留其他段）。"""
    import yaml

    data: dict[str, object] = {}
    if LOCAL_OVERRIDE_PATH.exists():
        try:
            file_data = load_yaml_file(LOCAL_OVERRIDE_PATH)
            if isinstance(file_data, dict):
                data = dict(file_data)
        except Exception:
            data = {}
