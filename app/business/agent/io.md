...
- **`await`（真异步）**：执行到 `await db.commit()` / `await llm.acreate()` 时，当前协程在 I/O 点**挂起**，
  把控制权交还事件循环；循环在这段时间去驱动其它协程；I/O 完成后交回结果，协程从 `await` 之后继续。
  → 循环始终可响应，`final` 能按时下发，`pending` 能按时复位。
- **阻塞同步调用（直接跑在循环线程）**：一个不 `await` 的长调用（如同步 LLM、同步 DB commit）会**占住循环线程整段时长**，
  循环无法切走，所有其它协程暂停 → 表现为 `final` 迟到、`pending` 不复位、新请求排队。
- **`asyncio.to_thread`（M1 方案）**：把阻塞工作丢到工作线程，循环线程继续跑其它协程。
