...
- **非图节点**：`persist` 不在 LangGraph 图中调用，而是在图运行**边界**（`CustomerServiceAgent.chat` / `chat_events` 拿到最终状态后）批量写入，保持图作为纯状态机、无副作用、可重放。
...
- **落库时机（边界批量）**：图运行期间（`input_normalizer → ... → context_compressor`）只读写内存态 `ConversationState`，不触碰存储；`chat` / `chat_events` 在图结束后调用 `MessageService.persist` 一次性写入用户消息 + 助手回复 + 状态快照。这样 SSE 断开不会丢用户消息，且图可被 checkpointer 重放。
