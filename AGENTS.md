...
- `rag/`：`chunking/`（策略模式分块：`models` / `base` / `recursive_splitter` / `structure_chunk` / **6 个策略文件**（markdown/word/json/excel_csv/pdf/ppt）/ `registry`）+ `retrieval/`（独立检索目录：`models` / `base` / `bm25.py`（单一 BM25 模块：`tokenize` / `build_sparse_vector` + 手搓倒排索引 `BM25Index` / `BM25Store` / `get_bm25_store` + 检索策略 `BM25Strategy`，合并于一处）/ `semantic` / `hybrid` / `rerank` / `registry`）+ `ingestion.py`（入库）。`chunking` 与 `retrieval` 互不 import。
